...
int main(int argc, char* argv[]) {
    std::string dataFilename { "data/set-skiplist-1m.txt" };
    ...
    for (unsigned ir = 0; ir < ratioList.size(); ir++) {
        ...
        for (unsigned it = 0; it < threadList.size(); it++) {
            ...
            std::cout << "\n----- Sets (Skiplist)   numElements=" << numElements << "   ratio=" << ratio/10. << "%   threads=" << nThreads << "   runs=" << numRuns << "   length=" << testLength.count() << "s -----\n";
            if (dsname == nullptr || std::strcmp(dsname, "hsskip-orcorig") == 0) {
				results[ic][it][ir] = bench.benchmark<HerlihyShavitLockFreeSkipListOrcGCOrig<UserWord>,UserWord>                      (cNames[ic], ratio, testLength, numRuns, numElements, false);
				ic++;
			}
            if (dsname == nullptr || std::strcmp(dsname, "hsskip-orc") == 0) {
				results[ic][it][ir] = bench.benchmark<HerlihyShavitLockFreeSkipListOrcGC<UserWord>,UserWord>                      (cNames[ic], ratio, testLength, numRuns, numElements, false);
				ic++;
			}
            maxClass = ic;
        }
